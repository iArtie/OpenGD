...
bool GameObject::init(std::string_view frame, std::string_view glowFrame)
{
	...
	{
		// NO usar Sprite::initWithFile aqui como fallback: eso crea una
		// textura suelta que no pertenece a ningun SpriteBatchNode, y al
		// intentar agregarla a _mainBatchNode*/_blendingBatchNode* revienta
		// el assert de SpriteBatchNode ("sprite->getTexture() ==
		// _textureAtlas->getTexture()"). Si el frame no existe en el atlas,
		// simplemente se descarta el objeto (como en el original).
		return false;
	}
...
void GameObject::customSetup()
{
	...
	if (childJson.empty()) {
		if (FileUtils::getInstance()->isFileExist("Custom/object.json")) {
			childJson = nlohmann::json::parse(FileUtils::getInstance()->getStringFromFile("Custom/object.json"));
		}
		else {
			childJson = nlohmann::json::object();
		}
	}

	if (childJson.contains(std::to_string(getID()))) {
		auto objJson = childJson[std::to_string(getID())];
		if (objJson.contains("default_z_order")) setGlobalZOrder((int)objJson["default_z_order"]);
		if (objJson.contains("default_z_layer")) _zLayer = (int)objJson["default_z_layer"];
		if (objJson.contains("default_primary_channel")) _mainColorChannel = (int)objJson["default_primary_channel"];
		if (objJson.contains("default_secondary_channel")) _secColorChannel = (int)objJson["default_secondary_channel"];

		if (objJson.contains("color_channel")) {
			if (objJson["color_channel"] == "base") _childSpritesChannel.push_back(0);
			else if (objJson["color_channel"] == "detail") _childSpritesChannel.push_back(1);
			else if (objJson["color_channel"] == "black") _childSpritesChannel.push_back(2);
		}
		else _childSpritesChannel.push_back(404);

		if (objJson.contains("object_type")) setGameObjectType((GameObjectType)objJson["object_type"]);
		if (objJson.contains("children")) addCustomSprites(objJson["children"], this);
	}
	...
	// Example particles for standard items
	switch (getID()) {
...
Color3B GameObject::getChannelColor(SpriteColor* colorChannel)
{
	...
	Color3B returnCol;
	if (colorChannel->_copyingColorID != -1) returnCol = getChannelColor(&bgl->_colorChannels[colorChannel->_copyingColorID]);
...
void GameObject::update()
{
	...
	if (!_isTrigger) this->setPosition(this->_startPosition + this->_startPosOffset);
	...
	switch (_childSpritesChannel.empty() ? -1 : _childSpritesChannel[0])
	{
	case 0:
		if (state == GroupProperties::GroupState::MAIN_ONLY || state == GroupProperties::GroupState::MAIN_DETAIL) _tempColor = groupColor;
		else applyColorChannel(this, 0, opacityMultiplier, _mainColor);

		if (_mainHSVEnabled) GameToolbox::applyHSV(_mainHSV, &_tempColor);
		if (getColor() != _tempColor) setColor(_tempColor);
		break;
	case 1:
		if (state == GroupProperties::GroupState::DETAIL_ONLY || state == GroupProperties::GroupState::MAIN_DETAIL) _tempColor = groupColor;
		else if (_secColor) applyColorChannel(this, 1, opacityMultiplier, _secColor);
		else applyColorChannel(this, 1, opacityMultiplier, _mainColor);

		if (_secondaryHSVEnabled) GameToolbox::applyHSV(_secondaryHSV, &_tempColor);
		else if (_isOnlyDetail) GameToolbox::applyHSV(_mainHSV, &_tempColor);
		if (getColor() != _tempColor) setColor(_tempColor);
		break;
	case 2:
	...
	for (size_t i = 0; i < _childSprites.size(); i++) {
		...
		case 0:
			if (state == GroupProperties::GroupState::MAIN_ONLY || state == GroupProperties::GroupState::MAIN_DETAIL) _tempColor = groupColor;
			else applyColorChannel(_childSprites[i], 0, opacityMultiplier, _mainColor);
			if (_mainHSVEnabled) GameToolbox::applyHSV(_mainHSV, &_tempColor);
			if (_childSprites[i]->getColor() != _tempColor) _childSprites[i]->setColor(_tempColor);
			break;
		case 1:
			if (state == GroupProperties::GroupState::DETAIL_ONLY || state == GroupProperties::GroupState::MAIN_DETAIL) _tempColor = groupColor;
			else if (_secColor) applyColorChannel(_childSprites[i], 1, opacityMultiplier, _secColor);
			else applyColorChannel(_childSprites[i], 1, opacityMultiplier, _mainColor);
			if (_secondaryHSVEnabled) GameToolbox::applyHSV(_secondaryHSV, &_tempColor);
			else if (_isOnlyDetail) GameToolbox::applyHSV(_mainHSV, &_tempColor);
			if (_childSprites[i]->getColor() != _tempColor) _childSprites[i]->setColor(_tempColor);
			break;
...
GameObject* GameObject::createFromString(std::string_view data)
{
	...
	for (size_t i = 0; i < properties.size() - 1; i += 2) {
		...
		case 17: if (obj->_isTrigger) dynamic_cast<EffectGameObject*>(obj)->_blending = GameToolbox::stoi(properties[i + 1]);
